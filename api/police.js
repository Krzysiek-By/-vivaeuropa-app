export default async function handler(req,res){
  ...
  for(const url of endpoints){
    try{
      ...
      const r=await fetch(url,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded;charset=UTF-8','user-agent':'VivaEuropa/0.90.51 (prototype police search)'},body:'data='+encodeURIComponent(q),signal:ctrl.signal});
      clearTimeout(timer);
      ...
      const items=(data.elements||[]).map(el=>({
        ...
        tags:el.tags||{}
      })).filter(x=>Number.isFinite(x.lat)&&Number.isFinite(x.lng)&&String(x.tags.access||'').toLowerCase()!=='private');
